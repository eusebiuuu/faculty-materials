...
def alpha_beta(alpha, beta, stare):
    ...
    if stare.j_curent == InfoJoc.JMAX:
        best_state.estimare = float("-inf")
        for mutare in stare.mutari():
            curr_state = alpha_beta(alpha, beta, mutare)
            if curr_state.estimare >= beta:
                best_state = curr_state
                break
            if alpha < curr_state.estimare:
                best_state = curr_state
                alpha = curr_state.estimare
    else:
        best_state.estimare = float("inf")
        for mutare in stare.mutari():
            curr_state = alpha_beta(alpha, beta, mutare)
            if curr_state.estimare <= alpha:
                best_state = curr_state
                break
            if beta > curr_state.estimare:
                best_state = curr_state
                beta = curr_state.estimare

    stare.stare_aleasa = best_state
    stare.estimare = stare.stare_aleasa.estimare
    return stare
