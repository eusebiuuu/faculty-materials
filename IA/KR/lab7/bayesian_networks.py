import copy
from queue import Queue


class Nod:
    def __init__(self, identificator, probabl=None,parinti=[], fii=[], tabel=[], observatie=None):
        self.identificator=identificator
        self.parinti=parinti
        self.fii=[]
        self.tabel=tabel
        self.probabl=probabl
        self.observatie=observatie

    def __str__(self):
        sir= f"{self.identificator}"
        if self.probabl is not None:
            sir+=f" prob: {self.probabl}"
        return sir
    
    def __repr__(self):
        sirTabel='\n'.join([str(linie) for linie in self.tabel])
        sir=f"Identificator: {self.identificator}\n"
        if self.probabl is not None:
            sir+=f"Probabilitate: {self.probabl}\n"
        sir+=f"Fii: {', '.join(self.fii)}\n"+ \
            f"Parinti: {', '.join(self.parinti)}\n"+ \
            f"Tabel probabilitati conditionate:\n{sirTabel}\n"
        

        return sir


class Retea:
    def __init__(self, fisier):
        self.graf={}

        f=open(fisier, "r")
        continut=f.read()
        textNoduri, auxInterog=continut.split("observatii:")
        textNoduri=textNoduri.strip()
        texteNoduri=textNoduri.split("nod:")
        for text in texteNoduri:
            if not text:
                continue
            liniiNod=text.strip().split("\n")
            identif=liniiNod[0].strip()
            probabilitate=None
            parinti=[]
            tabelProb=[]
            try:
                probabilitate=float(liniiNod[1])
            except:
                #nu am probabilitate data ci tabel de parinti
                parinti=liniiNod[1].split(", ")
                for linie in liniiNod[2:]:
                    linie=linie.strip().split()
                    linie=list(map(lambda x: True if x=="1" else False, linie[:-1]))+[float(linie[-1])]
                    tabelProb.append(linie)
            nodRetea=Nod(identificator=identif, probabl=probabilitate, parinti=parinti,tabel=tabelProb)
            self.graf[identif]=nodRetea
        textObservatii, varInterogare=auxInterog.strip().split("interogare:")
        textObservatii=textObservatii.strip()
        
        listaObservatii=[] if textObservatii=="#" else textObservatii.split("; ")
        self.observatii=[]
        for textObs in listaObservatii:
            if not textObs:
                continue
            identif, val=textObs.strip().split(":")
            val=val.strip()
            identif=identif.strip()
            self.observatii+=[identif]
            self.graf[identif].valoare=float(val)            
            self.graf[identif].observatie=1 if val=='1' else 0
        for k,nd in self.graf.items():
            for pr in nd.parinti:
                self.graf[pr].fii.append(k) 
        self.interogare=varInterogare.strip()


    def mark_free_subtrees(self, start_set, block_set):
        blocked_subtrees = {}
        nodes_info = Queue()
        count = {}

        for blocked_node in block_set:
            blocked_subtrees[blocked_node] = True

        for start_node in start_set:
            blocked_subtrees[start_node] = blocked_subtrees.get(start_node, False)
            nodes_info.put(start_node)
        
        while not nodes_info.empty():
            node_identif = nodes_info.get()

            for next in self.graf[node_identif].parinti:
                count[next] = count.get(next, 0) + 1

                blocked_subtrees[next] = blocked_subtrees.get(next, False) | blocked_subtrees[node_identif]

                if count[next] == len(self.graf[next].fii):
                    nodes_info.put(next)
        
        return blocked_subtrees


    def are_d_separated(self, source_set, dest_set, block_set):
        path = Queue()
        seen = {0: {}, 1: {}}
        leaves = []

        def add_to_path(dir_down, node, flag_up, flag_down):
            dir_up = not dir_down
            if dir_up and seen.get(0) and seen[0].get(node, False):
                return
            if dir_down and seen.get(1) and seen[1].get(node, False):
                return
            if dir_up:
                seen[0][node] = True
            if dir_down:
                seen[1][node] = True
            path.put((dir_down, node, flag_up, flag_down))

        for (identif, node) in self.graf.items():
            if len(node.fii) == 0:
                leaves.append(identif)

        blocked_subtree = self.mark_free_subtrees(leaves, block_set)

        for node in source_set:
            seen[0][node] = True
            seen[1][node] = True
            for next in self.graf[node].parinti:
                add_to_path(True, next, False, False)
            for next in self.graf[node].fii:
                add_to_path(False, next, False, False)
            
        
        while not path.empty():
            dir_down, node, flag_up, flag_down = path.get()
            
            if node in dest_set and not flag_down and not flag_up:
                return False
            
            if dir_down:
                if blocked_subtree[node]:
                    for next_down in self.graf[node].fii:
                        add_to_path(True, next_down, True, False)
                    
                    for next_up in self.graf[node].parinti:
                        add_to_path(False, next_up, False, False)
                
                else:
                    for next_up in self.graf[node].parinti:
                        add_to_path(False, next_up, False, True)

                    for next_down in self.graf[node].fii:
                        add_to_path(True, next_down, flag_up, False)
            else:
                if node in block_set:
                    for next_down in self.graf[node].fii:
                        add_to_path(True, next_down, True, False)
                    
                    for next_up in self.graf[node].parinti:
                        add_to_path(False, next_up, False, True)
                
                else:
                    for next_up in self.graf[node].parinti:
                        add_to_path(False, next_up, False, flag_down)

                    for next_down in self.graf[node].fii:
                        add_to_path(True, next_down, False, False)
        
        return True

    def interogare_retea(self,x):
        return self.suport_except(self.graf[x], None)

    def suport_except(self,x, v):
        alpha=1
        #if self.dovezi(x):
        if x.observatie is not None:
            #returnez doar probabilitatea pentru valoare True (fiinca false se calculeaza cu 1-probabl)
            return x.observatie 
        else:
            prob_e=self.dovezi_except(x,v)
            if not x.parinti:
                return alpha*prob_e*x.probabl
            else:
                l_prob=[]
                for p in x.parinti:
                    p_true=self.suport_except(self.graf[p], x)
                    l_prob.append(p_true)

                sumProb=0
                for linie in x.tabel:
                    prodProb=linie[-1]
                    for i,valAdev in enumerate(linie[:-1]):
                        prodProb*=l_prob[i] if valAdev else (1-l_prob[i])
                    sumProb+=prodProb
                
                return alpha*prob_e*sumProb

    def dovezi_except(self, x,v):
        y=list(x.fii)
        if v:
            y.remove(v.identificator)
        if not y:
            #return 0.5 
            return 1 #variabila booleana
        prod_prob=1
        for yi in y:
            yi=self.graf[yi]
            proabl_yi=self.dovezi_except(yi, None)
            zi=list(yi.parinti)
            zi_sters=zi.index(x.identificator)
            zi.remove(x.identificator)
            tabel_yi=copy.deepcopy(yi.tabel)
            for linie in tabel_yi:
                linie.pop(zi_sters)
            l_probz=[]
            
            for zij in zi:
                l_probz.append(self.suport_except(self.graf[zij], yi))
            sum_zi_true=0
            sum_zi_false=0
            for linie in tabel_yi:
                prodProb=1                
                for j,valAdev in enumerate(linie[:-1]):
                    prodProb*=l_probz[j] if valAdev else (1-l_probz[j])
                sum_zi_true+=prodProb*linie[-1]
                sum_zi_false+=prodProb*(1-linie[-1])
            sum_yi=proabl_yi*sum_zi_true+(1-proabl_yi)*sum_zi_false
            prod_prob*=sum_yi
        return prod_prob



    def __str__(self):
        sir="Noduri: \n"
        for nod in self.graf.values():
            sir+="NOD:"+repr(nod)+"-------------\n"
        sir+="Observatii: "+", ".join([str(self.graf[x]) for x in self.observatii])+"\n"
        sir+="Interogare: "+self.interogare
        return sir


r=Retea("input.txt")
rez=r.interogare_retea(r.interogare)
rez_fals=1-rez
print(str(r))
print(f"Pentru {r.interogare} probabilitatea să fie:\n" + \
       f"- True este {rez}\n" + \
       f"- False este {rez_fals}")

print(r.are_d_separated(["jocuri_calculator"], ["angajat"], ["timp_studiu_acasa"]))